    ...
    <div class="main-section">
      ...
      <div class="grid-container">
        ...
        <div
          ...
          :style="{ left: `${(currentPosition / cols) * 100}%` }"
        ></div>
      </div>
    </div>
...
const currentPosition = ref(0); // Position actuelle en colonnes
...
const startPlayback = () => {
  ...
  // Calculer l'intervalle basé sur le BPM (16th notes)
  // 60000ms / BPM / 4 (pour les 16th notes)
  const intervalMs = 60000 / tempo.value / 4;

  playbackInterval.value = setInterval(() => {
    currentPosition.value++;

    // Jouer les notes à la position actuelle
    playNotesAtPosition(currentPosition.value - 1);

    // Arrêter à la fin
    if (currentPosition.value >= cols) {
      stopPlayback();
    }
  }, intervalMs);
};
